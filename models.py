...
class RapportPDF(Base):
    ...
    forage = Column(String(500), nullable=True) # ---> LIGNE AJOUTÉE ICI <---
...
class Forage(Base):
    ...
    generatrices = relationship("Generatrice", back_populates="forage_parent")
    structures = relationship("EDZReel", back_populates="forage_parent")
    echantillons = relationship("Echantillon", back_populates="forage_parent")
    oxydations = relationship("Oxydation", back_populates="forage_lie")
    diagraphies = relationship("Diagraphie", back_populates="forage_ref")
    medias = relationship("Media", back_populates="forage_parent")
...
class Generatrice(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    # ⚠️ Clé étrangère pointant sur forage.forage
...
class Oxydation(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    # ⚠️ Dans ta base PostgreSQL, la colonne s'appelle "forage_" (avec un tiret du bas)
...
class TeneurEau(Base):
    ...
    humide_105 = Column(Float, nullable=True)
    humide_150 = Column(Float, nullable=True)
    sec_105 = Column(Float, nullable=True)
    sec_150 = Column(Float, nullable=True)
    
    hp_105 = Column(Float, nullable=True)
    hp_150 = Column(Float, nullable=True)
